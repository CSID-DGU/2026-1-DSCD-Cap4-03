...
export default function RoutinePage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const optCats = isFemale ? OPTIONAL_BY_GENDER.female : OPTIONAL_BY_GENDER.male;
        const optionalResults = await Promise.all(
          optCats.map((cat) => productApi.getList(cat).catch(() => [] as ProductSummary[]))
        );
        setOptionalProducts(optionalResults.flat());
      } catch (err) {
  ...
  const enrichedProducts: EnrichedProduct[] = currentRoutine.products
    .slice()
    .sort((a, b) => a.step - b.step)
    .map((rp) => {
      const detail = productMap.get(rp.product_id);
      if (!detail) return null;
      return { ...detail, applicationGuide: rp.application_guide, step: rp.step, timeTag: rp.time_tag };
    })
    ...
    <div className="rp-page">
      ...
      <div className="rp-body">
        ...
        <div className="rp-ai-desc-box">
          ...
          <p className="rp-ai-desc-text">{currentRoutine.ai_description}</p>
        </div>
        ...
        <div className="rp-routine-block">
          ...
          <div className="rp-steps-indicator">
            {enrichedProducts.map((p, idx) => (
              <div key={p.product_id} className="rp-step-indicator-item">
                <div className="rp-step-dot">{idx + 1}</div>
                <span>{p.category}</span>
                {idx < enrichedProducts.length - 1 && <div className="rp-step-line" />}
              </div>
            ))}
          </div>
